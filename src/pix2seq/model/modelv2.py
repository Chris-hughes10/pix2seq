...
class LlamaPix2Seq(nn.Module):
    def __init__(
        self,
        max_seq_len,
        image_size: int = 640,
        patch_size: int = 16,
        num_encoder_layers: int = 6,  # Kept to match TF exactly
        num_decoder_layers: int = 6,
        embedding_dim: int = 256,
        num_heads: int = 8,
        # n_kv_groups: int = 2,  # Default to 1/4 heads for GQA
        n_kv_groups=None,
        ...
        self.transformer_encoder = nn.ModuleList(
            [
                LlamaEncoderBlock(
                    ...
                    # num_kv_heads=self.n_kv_groups,
                    hidden_dim=dim_feedforward,
                    dropout=dropout,
                    is_causal=False,
                    context_length=encoder_seq_len,
                    rope_base=estimate_rope_theta(encoder_seq_len, embedding_dim),
                )
